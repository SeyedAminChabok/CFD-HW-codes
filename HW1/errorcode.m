...
for i=1:length(dx)
     ...
     % True error with considering analytical solution:
     True_error = abs(F2_Teta-F2_Teta_numerical);
     % Successive error without considering analytical solution:
     Successive_error = - diff (F2_Teta_numerical);
     hh = - diff (dx);
...
figure (1);
loglog(dx,True_error,'color','r');
xlabel('h');
ylabel('True error');

figure (2);
loglog(hh,Successive_error,'color','b');
xlabel('h');
